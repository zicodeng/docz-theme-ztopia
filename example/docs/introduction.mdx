...
_Write awesome documents for UI components with Docz and Ztopia theme!_

## Why Docz?

_It's never been easier to document your things!_

Let's all admit the fact that most engineers do not like writing docs,
needless to say writing awesome docs. We cannot really blame on them
for being lazy nor too smart to write some dummy text instead of code.
Given the fact that engineers are always too buzy with rushing projects to production,
fixing regression bugs, arguing with coworkers, it is vital to have a documentation
tool that requires no additional effort and time to write awesome docs.
...
before they can write awesome docs, whereas Docz fully leverages [MDX](https://mdxjs.com/),
allowing us to directly import UI components in markdown, making the documenting experience truly great.
Personally, I feel like writing docs with Storybook is more like writing JavaScript tests,
whereas writing docs with Docz is more like writing kindergarten journals.
...
- **Aesthetically pleasing design**: inspired by
  ...
  when I built this theme. Why? because I hope documentation with great UI&UX
  can attract people reading docs :)
...
- **One theme, two looks**: Ztopia theme comes with light and dark mode. You can click
  ...
  and use it to theme your components. See `example/Button.mdx` for an example
...
- **Gracefully handle complex prop shape**: it is common to have component props
  ...
  `PropTypes.shape({ firstName: PropTypes.string.isRequired, lastName: PropTypes.number, ...})`
  ...
  and see your complex shape. Check out examples in [Design](/docs/design) page
...
- **Playground with built-in iframe support**: the default Docz `<Playground>` is really cool,
  ...
  so that you can build your components in isolation. See [Button](/components/button) page for an example.
...
star Ztopia theme on GitHub. Your support is my biggest motivation :)
